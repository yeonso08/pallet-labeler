...
def main():
 p=argparse.ArgumentParser(description='PLY 자동 라벨링 (원본 보존)');p.add_argument('input',type=Path);p.add_argument('--output',type=Path,required=True);p.add_argument('--split',type=float,default=None);args=p.parse_args()
 model=joblib.load(Path(__file__).parent/'model.joblib');files=sorted(args.input.glob('*.ply')) if args.input.is_dir() else [args.input]
